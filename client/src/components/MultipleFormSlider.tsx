...
export default function MultipleFormSwiper({ forms }: { forms: FormWithCategory | undefined }) {
    console.log(forms);
                        ...
                        {forms?.map(form => (
                            <SwiperSlide key={form.id}>
                                <Link href={Route.form.formDetail(form.id)}>
                                    <div key={form.id} className="p-1 mb-7 flex flex-wrap items-center justify-center">
                                        <div className="flex-shrink-0 m-4 relative hover:scale-105 transition duration-500 cursor-pointer overflow-hidden bg-blue-400 rounded-lg max-w-xs shadow-lg" style={{ width: "250px", height: "300px" }}>
                                            ...
                                            <div className="relative pt-10 px-10 flex items-center justify-center">
                                                <div className="block absolute md:w-40 md:h-40 w-20 h-20 bottom-0 left-0 -mb-24 ml-3"
                                                    style={{ background: "radial-gradient(black, transparent 60%)", transform: "rotate3d(0, 0, 1, 20deg) scale3d(1, 0.6, 1)", opacity: "0.2" }}>
                                                </div>
                                                <img className="relative md:w-28 w-20" src={form.category.icon ? `${conf.categoryUrlPrefix}${form.category.icon}` : "/mostViewIcon.png"} alt={form.name} />
                                            </div>
